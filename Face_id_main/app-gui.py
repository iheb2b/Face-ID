...
class MainUI(tk.Tk):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.configure(background='#092233')
        global names
...
class StartPage(tk.Frame):
        ...
        def __init__(self, parent, controller):
            tk.Frame.__init__(self, parent, bg='#092233')
            ...
            img = tk.Label(self, image=render,background='#092233')
            img.image = render
            img.grid(row=0, column=10,  ipady=7, ipadx=70)
            label = tk.Label(self, text="        FACE ID        ", font=self.controller.title_font,fg='#fefffe',bg='#092233')
            label.grid(row=0, sticky="ew")
            button1 = tk.Button(self, text="   Add a User  ", fg="#ffffff", bg='#247588',command=lambda: self.controller.show_frame("PageOne"))
            button2 = tk.Button(self, text="   Check a User  ", fg="#ffffff",bg='#247588',command=lambda: self.controller.show_frame("PageTwo"))
            button3 = tk.Button(self, text="Quit", fg="#263942", bg="#ffffff", command=self.on_closing)
            button1.grid(row=2, column=0, pady=0, padx=10,sticky="ew")
            button2.grid(row=3, column=0, pady=0, padx=10,sticky="ew")
            button3.grid(row=4, column=0, pady=0, padx=10,sticky="ew")
...
class PageOne(tk.Frame):
    def __init__(self, parent, controller):
        tk.Frame.__init__(self, parent, bg='#092233')
        self.controller = controller
        tk.Label(self, text="Enter the name", fg="#ffffff",bg="#092233", font='Helvetica 12 bold').grid(row=0, column=0, pady=10, padx=5)
...
class PageThree(tk.Frame):
    ...
    def capimg(self):
        ...
        messagebox.showinfo("INSTRUCTIONS", "We will Capture 300 picture.")
        x = start_capture(self.controller.active_name)
        self.controller.num_of_images = x
        self.numimglabel.config(text=str(" images captured = "+str(x)))
